...
import {
  ...
  recognizeProduct,
  saveProductForBarcode
} from "../lib/api";
import type { GrossistMatch, ProductRecognition, RecognitionSource, UserRole } from "../types";
...
export function ScannerView({ role }: ScannerViewProps) {
  ...
  const [savedByUserName, setSavedByUserName] = useState<string | undefined>(undefined);
  const [manuellKorrigiert, setManuellKorrigiert] = useState<boolean | undefined>(undefined);
  ...
  function resetResult() {
    ...
    setSource(null);
    setGrossist(null);
    setSavedByUserName(undefined);
    setManuellKorrigiert(undefined);
    setActiveEan(null);
    setError(null);
    setSaveState("idle");
    if (previewUrl) URL.revokeObjectURL(previewUrl);
  ...
  async function handleCapture(file: File) {
    ...
    try {
      const scan = await recognizeProduct(file, projektTag);
      setResult(scan.result);
      setSource(scan.source);
      setGrossist(scan.grossist);
      setSavedByUserName(scan.savedByUserName);
      setManuellKorrigiert(scan.manuellKorrigiert);
      setActiveEan(null);
      setStatus("success");
    } catch (err) {
  ...
  async function handleBarcodeDetected(code: string) {
    ...
    try {
      const scan = await lookupProductByBarcode(code, projektTag);
      if (scan) {
        setResult(scan.result);
        setSource(scan.source);
        setGrossist(scan.grossist);
  ...
  async function handleSaveForBarcode() {
    ...
    try {
      ...
      setSavedByUserName(saved.savedByUserName);
      setManuellKorrigiert(saved.manuellKorrigiert);
      setActiveEan(pendingEan);
      setSaveState("saved");
    } catch {
      setSaveState("error");
    }
  ...
  async function handleCorrect(product: ProductRecognition) {
    if (!activeEan) return;
    const corrected = await correctProduct(activeEan, product);
    setResult(corrected.result);
    setGrossist(corrected.grossist);
    setSavedByUserName(corrected.savedByUserName);
    setManuellKorrigiert(corrected.manuellKorrigiert);
  }

  return (
    <>
      <label className="projekt-tag-field">
        Projekt/Baustelle (optional)
        <input
          value={projektTag}
          onChange={(e) => setProjektTag(e.target.value)}
          placeholder="z.B. Baustelle Musterweg 12"
        />
      </label>
      ...
      {mode === "foto" && <CameraCapture disabled={status === "loading"} onCapture={handleCapture} />}

      {status === "not_found" && pendingEan && (
        <div className="not-found-box">
          <p>
            📦 Barcode <strong>{pendingEan}</strong> ist noch nicht in der Team-Datenbank.
            <br />
      ...
      {previewUrl && <img className="preview-image" src={previewUrl} alt="Aufgenommenes Produkt" />}
      ...
      {status === "success" && result && source && (
        <>
          <ProductCard
            ...
            manuellKorrigiert={manuellKorrigiert}
            canCorrect={role === "admin" && source === "datenbank" && Boolean(activeEan)}
